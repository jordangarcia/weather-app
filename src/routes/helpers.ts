...
const computeOverallScore = (scores: {
  ...
  const minScore = Math.min(...Object.values(scores)) as Score
  const numMinScores = Object.values(scores).filter((score) => score === minScore).length
  if (minScore >= 3) {
    return minScore
  } else {
    // two 2s really are a 1
    if (numMinScores > 1) {
      return (minScore - 1) as Score
    }
    return minScore
  }
}
...
export const getCategoryEmoji = (key: string) => {
  return {
    // wind_score: '🌬',
    wind_score: '💨',
    temp_score: '🌡️',
    rain_score: '🌧️',
    // 'prev_rainfall_score': '🌧️',
  }[key]
}
